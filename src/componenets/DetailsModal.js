...
export default function TaskModal({
  ...
  postTask}) {
    ...
    function formatDate(dateString) {
      ...
      const month = String(date.getMonth() + 1).padStart(2, '0'); // Month is zero-based, so we add 1 and pad with leading zero if needed.
    ...
    const [state, setState] = React.useState({
      id: task ? task.id : '',
      name: task ? task.name : '',
      start: task ? formatDate(task.start) : new Date(),
      end: task ? formatDate(task.end) : new Date(),
    ...
    const startDateString = task.start
